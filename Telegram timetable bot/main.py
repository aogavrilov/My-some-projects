...
def get_text_messages(message):
    ...
    if message.text.lower() == "привет" or message.text.lower() == "/start":
        bot.send_message(message.from_user.id, "Привет, я бот, который поможет тебе с твоим расписанием.\n"
                                               ...
                                               "Чтобы посмотреть расписание на всю неделю, напиши /timetable\n"
                         )
            ...
            with connection.cursor() as cursor:
                ...
                for row in cursor:
                    message1 = message1 + row['timestart'] + "->" + row['timeend'] + ": " + row[
                        'Name'] + ", аудитория:" + row['classroom'] + ", ведет:" + row[
                                   'teacher'] + ", неделя: " + tmn + "\n"
                if(message1 != "Расписание на " + x[i] + ":"):
            ...
            try:
                ...
                for row in cursor:
                    if (int(row['odd']) == 0 or (int(row['odd']) % 2) == odd):
                        message1 = message1 + row['timestart'] + "->" + row['timeend'] + ": " + row[
                            'Name'] + ", аудитория:" + row['classroom'] + ", ведет:" + row['teacher'] + "\n"
                        print(row['timestart'] + "->" + row['timeend'] + ": " + row['Name'] + ", аудитория:" + row[
                            'classroom'] + ", ведет:" + row['teacher'] + "\n")
                bot.send_message(message.from_user.id, message1)
        ...
        tmp = ["monday", "tuesday", "wednesday", "thusday", "friday", "saturday", "sunday", "понедельник", "вторник",
